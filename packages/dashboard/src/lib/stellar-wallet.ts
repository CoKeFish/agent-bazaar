...
function friendlyError(e: unknown, fallback: string): string {
  ...
  if (ops.includes("op_no_trust"))
    return "Tu wallet de Agent Bazaar aún no tiene línea de confianza (trustline) de USDC. Haz una llamada a un agente primero o contáctanos.";
  if (ops.includes("op_underfunded"))
    return "Saldo USDC insuficiente en tu wallet externa.";
  if (ops.includes("op_line_full"))
    return "Tu wallet de Agent Bazaar no puede recibir más USDC (línea de confianza llena).";
